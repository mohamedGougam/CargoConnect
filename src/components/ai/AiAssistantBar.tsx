...
const EXAMPLE_PROMPTS = [
  ...
  "Find a vessel for 2,000 tons of steel from Greece to Egypt",
  "Show general cargo vessels near Piraeus",
  "Find capacity from Rotterdam to Alexandria",
];

const SUGGESTIONS = [
  "2,000t steel · Greece → Egypt",
  "General cargo near Piraeus",
  "Capacity · Rotterdam → Alexandria",
];
 ...
 */
export function AiAssistantBar({ onSubmit }: AiAssistantBarProps) {
  const [value, setValue] = useState("");
  const [focused, setFocused] = useState(false);
  const [exampleIndex, setExampleIndex] = useState(0);
  const [typedExample, setTypedExample] = useState("");

  const showPlaceholderAnimation = !focused && value.length === 0;
      ...
      <div className="pointer-events-auto w-full max-w-xl">
        ...
        <form
          ...
          className={`flex items-center gap-2 rounded-full border bg-[rgba(10,18,28,0.78)] px-2.5 py-1.5 shadow-[0_8px_28px_rgba(0,0,0,0.32)] backdrop-blur-md transition ${
            focused ? "border-teal-300/35" : "border-white/12"
          }`}
        >
          <span
          ...
          <div className="relative min-h-[1.35rem] min-w-0 flex-1">
            ...
            <input
              ...
              placeholder={focused || value ? "What are you looking to transport?" : ""}
              className="relative w-full bg-transparent text-[13px] text-white outline-none placeholder:text-slate-500"
              autoComplete="off"
            />
          </div>
          ...
          <button
            type="submit"
            className="shrink-0 rounded-full bg-teal-400/90 px-3 py-1 text-[11px] font-semibold tracking-wide text-slate-950 transition hover:bg-teal-300"
          >
            Ask
          </button>
        </form>
        ...
        <div className="mt-2 flex justify-center gap-1.5 overflow-hidden px-1">
          {SUGGESTIONS.map((prompt) => (
            <button
              ...
              className="max-w-[38%] truncate rounded-full border border-white/10 bg-white/[0.04] px-2.5 py-1 text-[10px] text-slate-300/90 transition hover:border-teal-300/30 hover:text-white"
            >
              {prompt}
            </button>
          ))}
