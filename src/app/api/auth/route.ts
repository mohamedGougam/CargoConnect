...
import { rateLimitedResponse, jsonOk } from "@/server/ops/errors";
import {
  ...
  return runWithRequestIdAsync(requestId, async () => {
    ...
    if (!originCheck.ok) {
      return NextResponse.json(
        { error: { code: originCheck.code, message: originCheck.error, requestId } },
    ...
    if (action === "signup") {
      const rl = await enforceRateLimit({
        policy: "signup",
        identityParts: [hashRateLimitIdentity(ip), hashRateLimitIdentity(email || "none")],
      });
      ...
      if ("error" in result) {
        return NextResponse.json(
          { error: { code: "SIGNUP_FAILED", message: result.error, requestId } },
          { status: 400, headers: { [REQUEST_ID_HEADER]: requestId } },
        );
      }
      metrics.authSignup();
    ...
    return NextResponse.json(
      { error: { code: "UNKNOWN_ACTION", message: "Unknown action", requestId } },
      { status: 400, headers: { [REQUEST_ID_HEADER]: requestId } },
    );
  });
}
